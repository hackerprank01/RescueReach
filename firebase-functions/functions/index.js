 ...
 */
exports.notifyEmergencyResponders = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    if (emergencyType) {
      respondersQuery = respondersQuery.where("services", "array-contains", emergencyType);
    }
    ...
    if (respondersSnapshot.empty) {
      ...
      return {success: false, notifiedCount: 0, message: "No matching responders found"};
    }
    ...
    respondersSnapshot.forEach((doc) => {
      ...
      notificationPromises.push(
          admin.messaging().send(notification)
              ...
              .catch((error) => {
                console.error("Error sending notification to responder:", doc.id, error);
              }),
      );
    });
    ...
    throw new functions.https.HttpsError("internal", "Error notifying responders", error);
  }
});
...
exports.sendEmergencySMS = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    if (!phoneNumbers || !phoneNumbers.length) {
      throw new functions.https.HttpsError("invalid-argument", "No phone numbers provided");
    }
...
${nearestServiceName ? `Nearest ${emergencyType}: ${nearestServiceName} (${nearestServiceDistance})` : ""}
...
This is an automated emergency alert. Please contact emergency services if needed.`;
    ...
    if (reportId) {
      await admin.firestore().collection("sos_reports").doc(reportId).update({
    ...
    throw new functions.https.HttpsError("internal", "Error sending emergency SMS", error);
  }
});
...
exports.notifyUserOnSOSStatusChange = functions.firestore
    ...
    .onUpdate(async (change, context) => {
      ...
      console.log(`Status changed from ${previousData.status} to ${newData.status} for report ${reportId}`);
      ...
      const userDoc = await admin.firestore().collection("users").doc(userPhone).get();
      ...
      try {
        ...
        console.log(`Notification sent to user ${userPhone} for report ${reportId}`);
...
exports.syncSOSStatusToRealtimeDB = functions.firestore
    ...
    .onUpdate(async (change, context) => {
      ...
      // Update realtime database
        await admin.database().ref(`active_sos/${reportId}/status`).set(newData.status);

        // If status is RESOLVED, move to archived_sos after a delay
        if (newData.status === "RESOLVED") {
          await admin.database().ref(`active_sos/${reportId}`).once("value", (snapshot) => {
            const sosData = snapshot.val();
            if (sosData) {
            // Add to archived_sos
              admin.database().ref(`archived_sos/${reportId}`).set({
                ...sosData,
                status: "RESOLVED",
